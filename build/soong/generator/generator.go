...
func init() {
	android.RegisterModuleType("nightshade_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = nightshadeExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = nightshadeExpandVariables(ctx, dep_file)
	...
	cmd := nightshadeExpandVariables(ctx, String(g.properties.Cmd))
